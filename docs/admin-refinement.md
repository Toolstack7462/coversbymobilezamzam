...
| Product editor: type templates                                                                                                                                              | **Not done.** A charger still shows the same fields as a phone case. This is the largest remaining gap in the brief.                                                                                                                                                                                    |
| Duplicate-product workflow                                                                                                                                                  | **Not done.**                                                                                                                                                                                                                                                                                           |
...
| Media manager refinements                                                                                                                                                   | **Not done.** Upload, reorder, primary selection and alt text are unchanged, and have no browser proof.                                                                                                                                                                                                 |
| Firefox and WebKit                                                                                                                                                          | **Not run.** Chromium only.                                                                                                                                                                                                                                                                             |
| Verification against the MariaDB runtime                                                                                                                                    | **Not done.** The browser suite runs against `wrangler dev` and D1. The Node/MariaDB server was verified separately (37 server tests, 1,792 rows reconciled, routes served) but the admin suite has not been pointed at it.                                                                             |

### What is still open, stated plainly

Four things, in the order a merchant would notice them:

1. **Product-type templates.** A charger's wattage and a case's device fit are
   different questions and the editor asks both of everything.
2. **Duplicate a product.** Adding the same case in a second colour is a full
   re-entry.
3. **The media manager.** Unrefined and untested through the browser.
4. **Cross-browser and MariaDB-runtime verification.** Chromium against D1 is
   what has actually been proven.

None of these is a correctness defect: nothing above is a decorative button, a
fake save or a bypassed permission. They are unfinished refinement, and they are
listed here so that "polished" is not claimed over them.
