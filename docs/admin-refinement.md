...
Three things, in the order a merchant would notice them:
...
3. **Cross-browser and MariaDB-runtime verification.** Chromium against
   `wrangler dev` and D1 is what has actually been proven.
...
| **Three workflow tests run on desktop only.** They write shared rows, and the two Playwright projects share one server and one database. Running them twice concurrently tests the scheduler, not the feature.      |
