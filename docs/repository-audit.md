...
about creating a repo _inside_ the project. Here the containment is an accident
of the home directory being a checkout, and giving the project its own repo is
what _prevents_ the leak.
...
as _candidates_ in `docs/known-limitations.md` for the merchant to confirm and
...
- The reference project itself flags an open question over whether **ZAM ZAM** is
  the _ragione sociale_ or a second brand. That is unresolved, and the legal name
