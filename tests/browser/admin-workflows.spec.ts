...
test.describe("the product editor", () => {
  ...
  test("refuses a save from a form that was open while somebody else saved", async ({
     ...
     *
     * Running on both projects meant two copies of this test saving the same
     * row while the product-type template test, over in the desktop project,
     * had that product open — so that test would occasionally have its save
     * refused by the very conflict guard this one is proving works, and fail
     * with a stale value that looked like a broken save.
     *
